#!/bin/sh
pid="SimplePseudonym"
clientId="SimplePseudonym_01"
clientDomain="SimplePseudonym"
accessableBy=""
...
function worker() {
	...
	curl --silent -X POST -H "x-client-id: $clientId" -H "x-client-domain: $clientDomain" -H "x-accessable-by: $accessableBy" -H 'Content-Type: text/csv' --data-binary @<(curl --silent -X GET -H "x-client-id: $clientId" -H "x-client-domain: $clientDomain" "http://localhost:3000/v1/transactions/$transactionId/processes/$processId/files/$fileId" | ./SimplePseudonym.sh -p-) "http://localhost:3000/v1/transactions/$transactionId/processes/$pid/files/$fileId"
}
